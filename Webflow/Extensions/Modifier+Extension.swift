...
fileprivate struct FloatingPanelModifier<PanelContent: View>: ViewModifier {
	...
	@State var panel: CommandBarView<PanelContent>?
	
	func body(content: Content) -> some View {
		content
			.onAppear {
				/// When the view appears, create, center and present the panel if ordered
				panel = CommandBarView(view: view, contentRect: contentRect, isPresented: $isPresented)
				panel?.center()
				if isPresented {
					present()
				}
...
extension View {
	 ...
	 **/
	func CommandBarView<Content: View>(isPresented: Binding<Bool>,
